...
const FOUNDER_OWNER = "0xc97785f7EEaBafFDE32436842AD4824cB4141f8b";
...
try {
  ...
  const founderOwner = process.env.FOUNDER_OWNER_ADDRESS || FOUNDER_OWNER;
  ...
  const deployData = encodeDeployData({ abi, bytecode, args: [founderOwner, agentOperator] });
  const deployGas = await publicClient.estimateGas({ account: account.address, data: deployData });
  ...
  const deployTxHash = await walletClient.deployContract({
    ...
    bytecode,
    account,
    args: [founderOwner, agentOperator],
  });
  ...
  const workflowOutput = {
    runId: `founder_owned_mainnet_receipt_${Date.now()}`,
    projectName: "Launch Desk Base Agent",
    readinessScore: 82,
    riskLevel: "medium",
    riskLevelCode: 2,
    proof: "Founder-owned Base mainnet AI workflow receipt contract deployed and first workflow proof recorded.",
    appUrl,
    owner: founderOwner,
    agentOperator,
  };
  ...
  const [owner, recordedAgentOperator, runCount] = await Promise.all([
    publicClient.readContract({ address: contractAddress, abi, functionName: "owner" }),
    publicClient.readContract({ address: contractAddress, abi, functionName: "agentOperator" }),
    publicClient.readContract({ address: contractAddress, abi, functionName: "runCount" }),
  ]);
  const afterWei = await publicClient.getBalance({ address: account.address });

  const deployment = {
    ok: true,
    mode: "deploy-workflow-proof-v3-mainnet",
    contract: "LaunchWorkflowProofV3",
    deployer: account.address,
    owner,
    agentOperator: recordedAgentOperator,
    network: { name: "Base Mainnet", chainId: base.id, rpcUrl },
    deployTxHash,
    contractAddress,
    ...
    founderOwned: owner.toLowerCase() === founderOwner.toLowerCase(),
  };
